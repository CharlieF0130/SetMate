...
class ApiService {
  ...
  static Future<List<CalendarSummary>> fetchCalendarSummary(String month) async {
  ...
  static Future<List<DailyTrainingDetail>> fetchAllTrainingsOnDate(DateTime date) async {
  final headers = await TokenService.getAuthHeader();
  final dateStr = DateFormat('yyyy-MM-dd').format(date);
  final response = await http.get(
...
static Future<bool> updateExercise(Exercise exercise) async {
  ...
  final response = await http.put(
    Uri.parse('$baseUrl/exercise/${exercise.exerciseId}'),
    headers: headers,
    body: json.encode({
